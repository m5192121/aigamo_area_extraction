/*
2016.11.13
・　画像のサイズとフィールドの大きさを指定する変数が同じものを使用していたため内外領域がおかしくなっていたので変数を分けた．（Line 20~21）
・　フィールドの大きさを入力するように変更．(Line 54~57)
・　内外判定領域が入力したフィールドの大きさに正しく対応できるように修正. (Line 242~245)
*/
...
static const int src_img_rows = 600;		//画像の幅
static const int src_img_cols = 600;		//画像の高さ
	...
	*/
	...
	for (int i = 1; i <= 212; i++){
		...
		*/
		...
		//選択した四隅を線で結ぶ
		line(img_wl, Point(Ax, Ay), Point(Bx, By), Scalar(200, 0, 0), 3);
		line(img_wl, Point(Bx, By), Point(Cx, Cy), Scalar(0, 200, 0), 3);
		line(img_wl, Point(Cx, Cy), Point(Dx, Dy), Scalar(200, 0, 0), 3);
		line(img_wl, Point(Dx, Dy), Point(Ax, Ay), Scalar(0, 200, 0), 3);
	
		imshow("drawing", img_wl);
		...
		*/
		...
		for (double i = 0; i <= src_img_cols; i += 100*w){
			for (double j = 0; j <= src_img_rows; j += 100*d){
				line(Eximg, Point(i, j), Point(i, width), Scalar(200, 200, 200), 3);
				line(Eximg, Point(i, j), Point(depth, j), Scalar(200, 200, 200), 3);
			}
		}

		//内側領域と外側領域の作成
		line(Eximg, Point(100*w, 100*d), Point(100*w, (depth-100)*d), Scalar(200, 0, 0), 2);
		line(Eximg, Point(100*w, 100*d), Point((width-100)*w, 100*d), Scalar(200, 0, 0), 2);	
		line(Eximg, Point((width-100)*w, 100*d), Point((width-100)*w,(depth-100)*d), Scalar(200, 0, 0), 2);
		line(Eximg, Point(100*w,(depth-100)*d), Point((width - 100)*w, (depth - 100)*d), Scalar(200, 0, 0), 2);
		/*
		line(Eximg, Point(0, 0), Point(0, src_img_cols), Scalar(0, 0, 0), CV_AA);
		line(Eximg, Point(0, 0), Point(src_img_rows, 0), Scalar(0, 0, 0), CV_AA);
		line(Eximg, Point(src_img_rows+100, 0), Point(src_img_rows+100, src_img_cols), Scalar(0, 0, 0), CV_AA);
		line(Eximg, Point(0, src_img_cols), Point(src_img_rows, src_img_cols), Scalar(0, 0, 0), CV_AA);
		*/
		...
		if (i == 1){
			...
			namedWindow("extraction area");
			imshow("extraction area",Eximg);
